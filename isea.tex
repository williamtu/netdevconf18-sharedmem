...
% Boris: no-drop-rate (RFC2544)
%  https://trex-tgn.cisco.com/trex/doc/trex_ndr_bench_doc.html
...
% todo
% why shaping doesn't work: packet tx buffer for shaping
% per-packet processing costs are not constant
\begin{document} 
\maketitle
...
Nvidia MLX5 & Dedicated RXQ\\ \hline
Intel ICE & Shared RXQs~\cite{icepatch} \\ \hline
Intel ICE (before~\cite{icepatch} & Dedicated RXQ \\ \hline
Broadcom BNXT & Shared RXQs~\cite{survey} \\ \hline
Netronome NFP & Shared RXQs~\cite{survey} \\ \hline
Solarflare SFC & Shared RXQs~\cite{survey} \\ \hline
Marvell Octeontx2 & Dedicated RXQ~\cite{octeontx2} \\ \hline
\end{tabular}
...
\subsection{New Devlink Eswitch Attribte: spool-mode}
Given the fairness issue, we'd like to propose a new devlink eswitch attribute
for users to enable or disable the Shared RXQs.
When memory is a precious resource such as SmartNIC or DPU, enabling the Shared RXQs
saves the most memory as it only uses the shared RXQs. And the memory consumption
does not grow linearly as the number of representors grows.
On the other hand, if users worry about potential fairness issue or need
performance isolation, then disabling shared RXQ and fallback to the dedicated
RXQ can solve the problem. Below is the proposed devlink, spool-mode:

\begin{itemize}
    \item none: Driver uses dedicated RXQ on its representor netdev.
    \item basic: Driver uses shared RXQs on its representor netdevs.
    \item spp (shared page pool): Driver uses shared page pool with adjustable RXQ depth on its representor netdevs. 
\end{itemize}
...
%https://trex-tgn.cisco.com/trex/doc/trex_ndr_bench_doc.html#_running_example
